...
class Request extends Component {
  constructor(props) {
    ...
    this.UnRequestFinalRejecte = this.UnRequestFinalRejecte.bind(this);
    this.UnRequestSuccess = this.UnRequestSuccess.bind(this);
    this.UnRequestFinalSuccess = this.UnRequestFinalSuccess.bind(this);
    ...
    Cron.getCron().then((res) => {
      ...
      if (res.data === true) {
        ...
        if (sessionStorage.getItem("role") === "validator") {
          ...
          PaidRequestService.getPaidRequest()
            .then((res) => {
              this.setState({ paidRequest: res.data });
            })
            .catch((ex) => {
          ...
          /**** */
          UnPaidRequestService.getUnPaidRequest()
            .then((res) => {
              this.setState({ unPaidRequest: res.data });
            })
            .catch((ex) => {
      ...
      .then((res) => {
        ...
        setTimeout(() => {
          ...
          const userDetaile =
            ...
            " - " +
            "Validation Request By Validator for the collaborator " +
  ...
  UnRequestFinalSuccess(id) {
    let Request = { statut: "accepted" };
    UnPaidRequestService.updateFinalStatutOfRequestAfterRHValidation(
      Request,
      id
    ).then((res) => {
      if (
  ...
  UnRequestRejecte(id) {
    ...
    UnPaidRequestService.statut(Request, id).then((res) => {
      if (
        ...
        UnPaidRquestService.getUnPaidRequest().then((res) => {
          this.setState({ unPaidRequest: res.data });
        });
      } else {
        UnPaidRequestService.getAllUnpaidRequestOfTheTeamOfValidator(
  ...
  ValidCancellationRequestMethod(collabid, paidreqid, balanceused) {
    ...
    setTimeout(() => {
      ...
      const userDetaile =
        ...
        JSON.parse(sessionStorage.getItem("lastname")) +
        " - " +
        " Validation Cancellation Request By " +
      ...
      this.state.col.solde = user1;
      // traitement balance action Cancellation
      //code changes in balance action treatment
      let BalanceAction = {
  ...
  //OPEN ADD HOLIDAYS Compnent
  //code separate traitment passe
  toAddHoliday = () => {
    this.setState({ showModal: true });
  };

  //TO GO INTO REQUEST PAGE
  //code separate traitment passe
  toRequest = () => {
    this.props.history.push("/admin/validator/Request");
  };

  //TO GO INTO ADD VACATION TYPE PAGE
  //code separate traitment passe
  ...
  render() {
    // console.log(allreq != null && allreq.length > 0 && allreq);
    return (
      <div>
        ...
        {/* code separate traitment passe */}
        {
          <AddRequestModal
            ...
            exceptionnalRequestOfCollaborateur={this.state.exeptionnel}
            recoveryRequestOfCollaborateur={this.state.RecoveryRequest}
            /***** */
            path="request"
            toRequest={this.toRequest}
            updateStateRequest={this.updateStateRequest}
            //code separate traitment passe
          />
        }
        ...
        <DeleteRequestModel
          ...
          unpaidrequestfinalrejecte={this.UnRequestFinalRejecte}
        />
        ...
        <ValidRequestModel
          ...
          unpaidrequestfinalsuccess={this.UnRequestFinalSuccess}
        />
